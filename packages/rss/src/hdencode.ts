...
export async function createHencodeDownloadable(item: HDEncodeItem): Promise<Downloadable> {
  if (!checked.includes(item.guid)) {
    checked.push(item.guid)
    ...
    if (info && info.poster) {
      ...
      const entity = await Entity.findOne({guid}) || Entity.create({guid})
      const newTags = [...(entity.tags || []), ...await Promise.all(linkInfo.categories.map(async (text: string) => await Tag.findOne({text}) || await Tag.create({text}).save()))]
      const finalTags: Tag[] = []
